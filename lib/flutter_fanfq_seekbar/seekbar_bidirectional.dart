...
///   },
/// )
class SeekBar_Bidirectional extends StatefulWidget {
...
class _SeekBarState extends State<SeekBar_Bidirectional> {
  ...
  @override
  void didUpdateWidget(SeekBar_Bidirectional oldWidget) {
    _value = widget.value > 1 ? 1 : widget.value < 0 ? 0 : widget.value;
    //_displayValue = (_value * _maxValue).round();
    // _secondValue = widget.secondValue > 1
    //     ? 1
    ...
    return GestureDetector(
      ...
      onHorizontalDragUpdate: (details) {
        ...
        if (widget.onProgressChanged != null) {
          ...
          widget.onProgressChanged((widget.maxValue * _value + widget.minValue - widget.maxValue/2).round());
        }
      },
...
class _SeekBarPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    canvas.drawLine(centerPoint, progressPoint, paint);

    ///画起始点
    final Paint startPaint = Paint()..isAntiAlias = true;
    startPaint.color = thumbColor;
    canvas.drawCircle(centerPoint, thumbRadius * 0.5, startPaint);
    ...
    final text = (maxValue * value + minValue - maxValue/2).round();

    final paragraphBuilder = ui.ParagraphBuilder(paragraphStyle)
      ..pushStyle(ui.TextStyle(color:Colors.blue))
      ..addText("${text}");//${displayValue}    ${(maxValue * value + minValue).round()}
